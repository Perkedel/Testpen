...
public sealed partial class PlayerControllerPlus : Component
{
	...
	void UpdateCameraPosition()
	{
		...
		var cam = CameraOverride ?? Scene.Camera as CameraComponent;
		if ( cam is null ) return;

		if ( ExtendedFeaturesEnabled && DisableCameraBinding )
			return;
		...
		if ( !IsAirborne && _eyez != 0 )
			eyePosition.z = _eyez.LerpTo( eyePosition.z, Time.Delta * (ExtendedFeaturesEnabled ? CrouchCameraSpeed : 50) );
