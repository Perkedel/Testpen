...
public class CustomFunctionNode : ShaderNodePlus, IErroringNode, IWarningNode, BaseNodePlus.IInitializeNode
{
	...
	private string GetInputResults( GraphCompiler compiler, out string error )
	{
		...
		foreach ( IPlugIn input in Inputs )
		{
			...
			{
				if ( input.Type == typeof( Texture ) || input.Type == typeof( Sampler ) )
				{
