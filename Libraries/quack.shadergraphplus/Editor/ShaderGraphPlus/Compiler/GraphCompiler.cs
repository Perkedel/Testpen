...
public sealed partial class GraphCompiler
{
	...
	public NodeResult Result( NodeInput input )
	{
		...
		if ( node is VoidFunctionNode voidFunctionNode )
		{
			voidFunctionNode.Register( this );

			if ( !ShaderResult.VoidLocals.ContainsKey( node.Identifier ) )
			{
				voidFunctionNode.RegisterVoidFunction( this );
			}

			if ( ShaderResult.VoidLocals.TryGetValue( node.Identifier, out VoidData data ) && !data.AlreadyPostProcessed )
			{
				var functionCall = data.FunctionCall;

				foreach ( var target in data.TargetProperties )
				{
					var nodeInputProperty = node.GetType().GetProperty( target.Key.targetProperty );

					if ( nodeInputProperty == null && nodeInputProperty.PropertyType != typeof( NodeInput ) )
					{
						SGPLogger.Error( $"\"{nodeInputProperty.PropertyType}\" is not of type \"{typeof( NodeInput )}\"" );
						continue;
					}

					var inputAttribute = nodeInputProperty.GetCustomAttribute<BaseNodePlus.InputAttribute>();
					var titleAttribute = nodeInputProperty.GetCustomAttribute<TitleAttribute>();
					...
					var nodeInput = (NodeInput)nodeInputProperty.GetValue( node, null );
					...
					if ( !ValueTypesNoDefault.Contains( inputAttribute.Type ) )
					{
						...
						PropertyInfo defaultPropertyInfo = node.GetType().GetProperty( target.Value );
						...
						if ( !string.IsNullOrWhiteSpace( target.Value ) )
						{
							if ( defaultPropertyInfo != null )
							{
								...
								{
									throw new Exception( $"Default value \"{target.Value}\" does not match type of \"{title}\" node input!" );
								}
							}
							else
							{
								throw new Exception( $"Could not find property with the name \"{target.Value}\"" );
							}
						}
					...
					functionCall = functionCall.Replace( target.Key.placeholderName, funcResult.Code );
				}
		...
		{
			if ( IsVs && customFunctionNode.PixelStageOnly )
			{
				NodeErrors[node] = ["This custom function node is set to be used in the pixel stage only!"];
			...
			if ( !voidData.IsValid )
			{
				SGPLogger.Error( $"Couldnt find VoidData in dictionary!", IsPreview );
